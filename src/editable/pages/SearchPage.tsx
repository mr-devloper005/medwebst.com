...
function SearchResultCard({ post, index }: { post: SitePost; index: number }) {
  ...
  return (
    <Link href={href} className={`group block overflow-hidden border border-[var(--editable-border)] bg-white transition hover:-translate-y-1 hover:shadow-2xl ${strong ? 'md:col-span-2' : ''}`}>
      {image ? (
      ...
      <div className="p-5 sm:p-6">
        ...
        <h2 className="editable-display mt-4 line-clamp-3 text-2xl font-bold leading-tight text-[var(--editable-page-text,#211713)]">{post.title}</h2>
...
export default async function SearchPage({ searchParams }: { searchParams?: Promise<{ q?: string; category?: string; task?: string; master?: string }> }) {
  ...
  return (
    <EditableSiteShell>
      <main className="min-h-screen bg-white text-[var(--editable-page-text,#121212)]">
        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="grid gap-8 border border-[var(--editable-border)] bg-[var(--slot4-panel-bg)] p-6 md:grid-cols-[0.8fr_1.2fr] lg:p-10">
            <div>
              ...
              <h1 className="editable-display mt-5 text-5xl font-bold leading-tight sm:text-6xl">{pagesContent.search.hero.title}</h1>
            ...
            <form action="/search" className="self-end border border-[var(--editable-border)] bg-white p-4 sm:p-5">
              ...
              <button className="mt-3 inline-flex h-12 w-full items-center justify-center bg-[var(--slot4-accent)] px-6 text-sm font-black uppercase tracking-[0.18em] text-white transition hover:-translate-y-0.5" type="submit">Search</button>
            </form>
          </div>
