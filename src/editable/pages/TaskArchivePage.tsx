...
const taskGrid: Record<TaskKey, string> = {
  article: 'grid gap-x-8 gap-y-12 sm:grid-cols-2 lg:grid-cols-4',
...
const cardBase = 'group block border border-[var(--tk-line)] bg-[var(--tk-surface)] transition duration-500 hover:-translate-y-1 hover:shadow-[0_24px_54px_rgba(18,18,18,0.12)]'
      ...
      <main style={taskThemeStyle(task)} className="min-h-screen bg-[var(--tk-bg)] text-[var(--tk-text)]">
        <header className="relative overflow-hidden border-b border-[var(--tk-line)] bg-white">
          <div className="relative mx-auto max-w-[var(--editable-container)] px-4 py-12 sm:px-6 sm:py-16 lg:px-8">
            <div className="flex items-center gap-3 text-xs font-bold uppercase tracking-[0.18em] text-[var(--tk-accent)]">
              <span>{theme.kicker}</span>
              <span className="h-1 w-1 rounded-full bg-[var(--tk-accent)] opacity-60" />
              <span className="text-[var(--tk-muted)]">{label}</span>
            </div>
            <h1 className="editable-display mt-5 max-w-4xl text-balance text-[2.4rem] font-bold leading-[1.1] sm:text-5xl lg:text-6xl">
            ...
            <div className="mt-12 flex flex-col gap-4 border-t border-[var(--tk-line)] pt-6 sm:flex-row sm:items-center sm:justify-between">
              ...
              <form action={basePath} className="flex items-center gap-2.5">
                ...
                <button className="inline-flex h-11 items-center bg-[var(--tk-accent)] px-5 text-sm font-semibold text-[var(--tk-on-accent)] transition hover:opacity-90">Apply</button>
              </form>
            </div>
          </div>
        </header>
...
function ArticleArchiveCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  ...
  const horizontal = index % 7 === 0 && !featured
  return (
    <Link href={href} className={horizontal ? 'group grid gap-4 border-t border-[var(--tk-line)] py-5 sm:col-span-2 sm:grid-cols-[220px_minmax(0,1fr)]' : `group block ${featured ? 'sm:col-span-2' : ''}`}>
      <div className={`${horizontal ? 'aspect-[16/10]' : featured ? 'aspect-[16/8]' : 'aspect-[16/9]'} overflow-hidden bg-[var(--tk-raised)]`}>
        {image ? <img src={image} alt="" className="h-full w-full object-cover transition duration-700 group-hover:scale-[1.03]" /> : <div className="flex h-full items-center justify-center bg-[linear-gradient(135deg,#172a3d,#d21f86)] text-sm font-bold uppercase tracking-[0.16em] text-white">medwebst insight</div>}
      </div>
      <div className={horizontal ? 'py-1' : 'pt-3'}>
        <div className="flex items-center gap-2 text-sm text-[var(--tk-text)]">
          <span>{category}</span>
          <span className="text-[var(--tk-muted)]">· No. {String(index + 1).padStart(2, '0')}</span>
        </div>
        <h2 className={`editable-display mt-2 font-bold leading-tight ${featured ? 'text-4xl' : 'text-2xl'}`}>{post.title}</h2>
        <p className="mt-2 line-clamp-3 text-sm leading-6 text-[var(--tk-muted)]">{getSummary(post)}</p>
        <span className="mt-3 inline-flex text-sm underline decoration-[var(--tk-accent)] underline-offset-4">Read article</span>
      </div>
    </Link>
  )
}
  ...
  return (
    <Link href={href} className={`${cardBase} flex items-center gap-5 p-5 sm:p-6`}>
      ...
      <div className="min-w-0 flex-1">
        <h2 className="editable-display truncate text-2xl font-bold">{post.title}</h2>
  ...
  return (
    <Link href={href} className="group mb-5 block break-inside-avoid overflow-hidden rounded-[var(--tk-radius)] border border-[var(--tk-line)] bg-[var(--tk-surface)] transition duration-300 hover:-translate-y-1">
      <div className={`relative overflow-hidden ${index % 3 === 0 ? 'aspect-[3/4]' : 'aspect-[4/3]'}`}>
        {image ? <img src={image} alt="" className="h-full w-full object-cover transition duration-700 group-hover:scale-[1.04]" /> : <div className="flex h-full items-center justify-center bg-[linear-gradient(135deg,#172a3d,#d21f86)] text-sm font-bold uppercase tracking-[0.16em] text-white">medwebst gallery</div>}
